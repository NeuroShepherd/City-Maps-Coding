...
```{r}
# Download NorCal shapefile information;  only complete if files not already downloaded to the ShapeFile Storage folder
if (!dir.exists("../ShapeFile Storage/shapefiles_SF")) {
  url <- "http://download.geofabrik.de/north-america/us/california/norcal-latest-free.shp.zip"
  setwd("../ShapeFile Storage")
  curl::curl_download(url, destfile="NorCal_shapes.shp.zip")
  unzip("NorCal_shapes.shp.zip", exdir="shapefiles_SF")
}
```

```{r}
# Read in data files
setwd("../ShapeFile Storage/shapefiles_SF")
...
```{r}
# Select San Francisco boundaries
# OSM lookup https://nominatim.openstreetmap.org/search.php?q=san+francisco&polygon_geojson=1&viewbox=
places_san_francisco <- places_import %>%
    filter(osm_id == 111968)

roads_cropped <- st_intersection(roads_import, places_san_francisco)
water_cropped <- st_intersection(water_import, places_san_francisco)
waterways_cropped <- st_intersection(waterways_import, places_san_francisco)
railways_cropped <- st_intersection(railways_import, places_san_francisco)
```
...
ggsave("San_Francisco_map.png", width = 297, height = 420, units = "mm", dpi = "retina")
