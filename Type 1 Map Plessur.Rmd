...
```{r}
# Download Switzerland shapefile information; only complete if files not already downloaded to the ShapeFile Storage folder
if (!dir.exists("../ShapeFile Storage/shapefiles_Schweiz")) {
  url <- "http://download.geofabrik.de/europe/switzerland-latest-free.shp.zip"
  setwd("../ShapeFile Storage")
  curl::curl_download(url, destfile="Schweiz_shapes.shp.zip")
  unzip("Schweiz_shapes.shp.zip", exdir="shapefiles_Schweiz")
}
```

```{r}
# Read in data files
setwd("../ShapeFile Storage/shapefiles_Schweiz")
...
```{r}
# Select Plessur boundaries
# OSM lookup https://nominatim.openstreetmap.org/search.php?q=san+francisco&polygon_geojson=1&viewbox=
places_plessur <- places_import %>%
    filter(osm_id == 2505682)

roads_cropped <- st_intersection(roads_import, places_plessur)
water_cropped <- st_intersection(water_import, places_plessur)
waterways_cropped <- st_intersection(waterways_import, places_plessur)
railways_cropped <- st_intersection(railways_import, places_plessur)
```
...
ggsave("Plessur_map.png", width = 297, height = 420, units = "mm", dpi = "retina")
```
